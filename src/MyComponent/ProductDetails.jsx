...
import { useCartOperations } from "./hooks/useCartOperations"; // Import the custom hook
import ProductCard from "./common/ProductCard"; // Import the reusable ProductCard
import  Nav  from "./Nav";
export default function ProductDetails() {
  const location = useLocation();
  const navigate = useNavigate();
  const item = location.state;
  const { addingToCart, handleAddToCart, handleBuyNow } = useCartOperations(); // Use the custom hook

  if (!item) return <div>No product found</div>;

  const recommended = Bestseller.filter(p => p.id !== item.id).slice(0, 4);
  const isAdding = addingToCart[item.id]; // Get loading state for current product
